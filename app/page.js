...
export default function HomePage() {
  ...
  const loadStats = async () => {
    const { data, error } = await supabase
      .from('documents')
      .select('status')
    if (error) { console.error('Ошибка загрузки статистики:', error); return }
    setStats({
      total: data.length,
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      if (!cancelled && hasActive) setTimeout(tick, 5000)
    }
    tick()
  ...
  const handleDelete = async (taskId) => {
    try {
      const { data: docs } = await supabase.from('documents').select('file_path').eq('task_id', taskId)
  ...
  return (
    <div style={{ maxWidth: 920, margin: '0 auto', padding: '28px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600, color: '#1C6B41', background: '#ECF6EF', padding: '7px 14px', borderRadius: 999 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 32 }}>
        {[
          ...
          <div key={s.label} style={{ background: 'white', borderRadius: 18, padding: '20px 22px', boxShadow: '0 1px 2px rgba(22,32,26,0.04), 0 0 0 1px rgba(22,32,26,0.04)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <p style={{ fontSize: 11, fontWeight: 600, color: '#9CA6A0', textTransform: 'uppercase', letterSpacing: '0.07em', margin: 0 }}>
          Последние загрузки
        </p>
        <button onClick={() => router.push('/upload')} style={{ fontSize: 13, fontWeight: 600, color: 'white', background: '#16201A', border: 'none', borderRadius: 9, padding: '8px 16px', cursor: 'pointer' }}>
          + Новая загрузка
        </button>
      </div>

      {loadingTasks ? (
        <div style={{ padding: 24, textAlign: 'center', color: '#9CA6A0', fontSize: 14 }}>Загрузка...</div>
      ) : (
