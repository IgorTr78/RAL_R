...
import { FileText, ChevronRight, RefreshCw, Trash2, Clock } from 'lucide-react'
...
function formatDuration(createdAt, completedAt) {
  ...
  const sec = (new Date(completedAt) - new Date(createdAt)) / 1000
...
export default function TaskList({ tasks = [], onRefresh, onDelete }) {
  ...
  return (
    <div style={{ background: 'white', borderRadius: 20, overflow: 'hidden', boxShadow: '0 1px 2px rgba(22,32,26,0.04), 0 0 0 1px rgba(22,32,26,0.04)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 24px', borderBottom: '1.5px solid #F0F2F0' }}>
        <span style={{ fontSize: 15.5, fontWeight: 800, color: '#16201A', letterSpacing: '-0.01em' }}>История загрузок</span>
        <button onClick={onRefresh} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12.5, color: '#16201A', background: '#F6F7F6', border: 'none', borderRadius: 10, padding: '7px 14px', cursor: 'pointer', fontWeight: 700 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
            <thead>
              <tr style={{ background: '#FAFBFA' }}>
                {['#', 'Файл', 'Дата', 'Документов', 'Статус', 'Время', 'Действие', ''].map(h => (
                  <th key={h} style={{ padding: '12px 24px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#9CA6A0', letterSpacing: '0.08em', textTransform: 'uppercase', borderBottom: '1.5px solid #F0F2F0', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {tasks.map((task, i) => {
                const duration = formatDuration(task.raw_created_at, task.completed_at)
                return (
                  <tr key={task.id} style={{ borderBottom: '1.5px solid #F6F7F6' }}>
                    ...
                    <td style={{ padding: '14px 24px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                        <div style={{ background: '#ECF6EF', borderRadius: 10, width: 34, height: 34, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <td style={{ padding: '14px 24px' }}>
                      {duration ? (
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12.5, fontWeight: 600, color: '#3A453E' }}>
                          <Clock size={12} color="#9CA6A0" />{duration}
                        </span>
                      ) : (
                    ...
                    <td style={{ padding: '14px 24px' }}>
                      {task.status === 'done' || task.status === 'partial' ? (
                        <button onClick={() => router.push(`/results/${task.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12.5, padding: '7px 16px', borderRadius: 10, border: 'none', color: 'white', background: '#16201A', cursor: 'pointer', fontWeight: 700 }}>
                    ...
                    <td style={{ padding: '14px 24px' }}>
                      <button
                        onClick={() => { if (confirm(`Удалить задачу "${task.filename}"?`)) onDelete && onDelete(task.id) }}
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 30, height: 30, borderRadius: 10, border: 'none', color: '#C0392B', background: '#FDEEEE', cursor: 'pointer' }}
                      >
                        <Trash2 size={14} />
                      </button>
                    </td>
